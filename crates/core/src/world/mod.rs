...
impl World {
    ...
    /// 按配置比例构建地形分配表（百分位映射）
    fn build_terrain_assignment(total_cells: u32, ratios: &std::collections::BTreeMap<String, f32>) -> Vec<TerrainType> {
    ...
    /// 根据地形匹配资源类型
    fn terrain_match_resource(terrain: TerrainType, rng: &mut impl rand::Rng) -> ResourceType {
        match terrain {
            TerrainType::Forest => {
