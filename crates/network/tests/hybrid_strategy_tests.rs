...
async fn test_peer_id_generation() {
    ...
    let transport2 = Libp2pTransport::new().unwrap();
...
async fn test_load_from_file_generates_new_key() {
    // 使用不存在的文件路径，应该生成新密钥
    let transport = Libp2pTransport::load_from_file("/nonexistent/key/file.bin");
    assert!(transport.is_ok());
}
...
async fn test_save_and_load_key() {
    ...
    // 从文件加载
    let loaded_transport = Libp2pTransport::load_from_file(temp_path);
