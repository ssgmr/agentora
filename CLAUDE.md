...
cargo test --test responsibility_boundary_tests
cargo test --test tier2_action_tests
cargo test --test tier3_emergence_tests
cargo test --test prompt_feedback_tests
...
- **`NullMessageHandler`** — 空消息处理器（gossip模块）
- **混合传输策略** — TCP直连 + DCUtR打洞 + Relay穿透
- **NAT穿透** — AutoNAT探测 + DCUtR Hole Punching
- **KAD DHT** — 节点发现 + 区域订阅
- **区域订阅** — 基于空间位置的Topic订阅
- **新增模块**：`behaviour.rs`（复合Behavior）、`swarm.rs`（Swarm管理）、`config.rs`（网络配置）、`nat.rs`（NAT穿透）

### Godot客户端 (`client/`)
Godot 4 GDScript客户端，负责渲染和交互：
- `main.gd` — 主场景入口
- `bridge_accessor.gd` — 桥接访问器：与SimulationBridge GDExtension节点交互
- `state_manager.gd` — 状态管理器：管理游戏状态（Agent过滤、世界事件、叙事频道）
...
- `narrative_feed.gd` — 叙事流面板（支持多频道过滤）
- `agent_detail_panel.gd` — Agent详情面板
- `milestone_panel.gd` — 文明里程碑面板
- `agent_filter.gd` — Agent过滤UI（按状态/类型筛选）
- `channel_selector.gd` — 叙事频道选择器
- `setup_wizard.gd` — 启动设置向导（LLM/Agent/P2P配置）
- `settings_panel.gd` — 设置面板（运行时配置调整）
- `p2p_panel.gd` — P2P网络状态面板
- `shared_ui_styles.gd` — 共享UI样式工具
...
- **`types.rs`** — 核心类型：AgentId, Position, Direction, ResourceType, TerrainType, StructureType, ActionType(15种), Action, PersonalitySeed, PeerId
...
- **`prompt.rs`** — PromptBuilder：分级截断（策略→记忆→感知），最大6000 tokens，中文感知估算
...
- **`agent_loop.rs`** — Agent决策循环6阶段流水线：WorldState构建 → 感知 → 决策 → 应用 → Delta发送 → 叙事发送（**决策与apply在同一task内同步执行**）
...
- **`delta_dispatcher.rs`** — DeltaDispatcher：双通道分发（本地mpsc + P2P GossipSub）
...
- **`mod.rs`** — Agent实体（信任关系/临时偏好/frozen_inventory/pending_trade_id/XP等级系统/震荡检测）
...
- **`mod.rs`** — World主结构（256x256地图、区域、资源、建筑、压力池、Agent管理、ShadowAgent远程代理、对话日志、Agent位置反向索引、统计计数器）
- **`map.rs`** — CellGrid地形网格
- **`region.rs`** — 区域划分和管理
...
- **`actions/`** — 动作执行路由（mod.rs分发 + movement/survival/social/legacy子模块，15种动作类型）
- **`snapshot.rs`** — 世界快照生成
...
GDExtension桥接Rust核心引擎到Godot客户端（7个模块）：
...
- **`icon_processor.rs`** — 图标处理：用户自定义图标缩放到32x32（Lanczos3过滤器），支持png/jpg/webp/bmp格式
- **`user_config.rs`** — 用户配置：引导页面配置管理（LLM模式local/remote/rule_only、Agent名称/图标、P2P模式single/create/join）
...
- **Windows打包DLL位置**：Godot导出的exe运行时从exe同级目录加载DLL，打包脚本需复制到`build/`根目录**和**`build/client/bin/`两个位置
...
- `TokenBudget`：总量控制 ≤3000 chars（可配置：total=3000, chronicle=1200, db=1000, strategy=800）
- `ShortTerm`：短期记忆队列（容量8）
- `Fence`：记忆围栏/隔离
...
- 最大6000 tokens（prompt_max_tokens），中文感知估算
...
### 测试结构 (`tests/`)
- **单元测试**：`decision_tests`, `crdt_tests`, `json_parse_tests`, `strategy_tests`, `memory_tests`, `legacy_tests`, `agent_tests`, `responsibility_boundary_tests`, `tier2_action_tests`, `tier3_emergence_tests`, `prompt_feedback_tests`, `bridge_config_tests`
...
- 决策prompt控制在 6K tokens内（prompt_max_tokens=6000），记忆预算max 3000 chars（可配置）
...
- 支持Anthropic兼容端点作为备用
- 决策max_tokens=1024，temperature=0.7，prompt_max_tokens=6000
- 默认模型：`qwen3.5-4b@q4_k_m`（主），备用 `gemma-4-e2b-it`
- 记忆系统配置在 `[memory]` section，包含预算/存储/检索/容量/Prompt约束等参数
  - `total_budget=3000` chars, `chronicle_budget=1200`, `db_budget=1000`, `strategy_budget=800`
  - 检索层：`importance_threshold=0.5`, `search_limit=5`, `snippet_max_chars=300`
  - 短期记忆容量：`short_term_capacity=8`
  - 编年史存储上限：`chronicle_limit=1800`，世界种子限制：`world_seed_limit=500`

**模拟配置** (`config/sim.toml`)
- `initial_agent_count=1`, `npc_count=2`（实际玩家只有一个Agent，NPC为开发测试用）
- `tick_interval_secs=5`, `npc_decision_interval_secs=3`, `player_decision_interval_secs=2`
- `vision_radius=10`, `trade_timeout_ticks=50`
- 物品栏配置：`max_slots=20`, `max_stack_size=20`, `warehouse_limit_multiplier=2`
