...
| memory-system | core | 三级记忆（短期/中期压缩/长期索引）、遗忘衰减 |
...
2. SPARK
   MotivationVector + WorldSatisfaction → gap_calculation → Spark
                                                          ↓
3. BUILD PROMPT
   Spark + Perception + Memory.compress() + Social.recent() → Prompt (<2.5K tokens)
                                                                ↓
4. LLM GENERATE
   Prompt → LlmProvider.generate() → raw_text → JSON parse → Vec<ActionCandidate>
                                                                    ↓
5. VALIDATE & SELECT
   ActionCandidate[] → RuleEngine.validate() → valid[] → motivation_weighted_select() → Action
                                                                                          ↓
6. EXECUTE
   Action → World.apply() → Effects → CRDT.generate_ops() → GossipSub.broadcast()
                     ↓
7. ECHO
   Effects → Agent.update_memory() + Agent.evolve_motivation() → 新状态
                     ↓
8. SNAPSHOT
...
- [ ] 具体选用哪个GGUF模型（Qwen3-1.7B / Gemma-4-2B 等），等模型调研完成后确定
