...
**来自** `openspec/specs/godot-client/spec.md` 中的 "GDExtension Bridge" 需求

**MODIFIED**：原文要求 SimulationBridge 在 Godot 节点中"启动Tokio运行时、初始化World/Network/Sync/AI模块"，现在明确实现路径为：通过 `crates/bridge/src/lib.rs` 的 Rust GDExtension 类实现，而非 GDScript。autoload 配置从 `res://scripts/simulation_bridge.gd` 切换为 GDExtension 注册的类型。
