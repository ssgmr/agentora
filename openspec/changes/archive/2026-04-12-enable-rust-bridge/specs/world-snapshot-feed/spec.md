...
### Requirement: GDScript 模拟版 SimulationBridge

**原因**：GDScript 模拟版（`res://scripts/simulation_bridge.gd`）仅为占位实现，Agent 行为完全随机，不具备真实模拟逻辑。Rust GDExtension 版本提供完整的 World/Agent/决策管道。

**迁移方案**：将 `project.godot` autoload 配置从 GDScript 路径移除。保留 GDScript 文件作为 GDExtension 加载失败时的回退方案（运行时检测 DLL 是否存在）。
